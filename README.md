...
```
NuTrackDemo03/
├── 🎯 NuTrackDemo03.xcodeproj/          # Xcode 專案設定
│   └── xcuserdata/                      # 使用者特定設定
│       └── Guest.xcuserdatad/
│           └── xcschemes/
│               └── xcschememanagement.plist
├── 📱 NuTrackDemo03/                    # 主要應用程式目錄
│   ├── NuTrackDemo03App.swift           # 應用程式入口點
│   ├── NuTrackDemo03.entitlements       # 應用程式權限設定
│   ├── 📊 Models/                       # 資料模型層
...
│   └── 🛠️ Extensions/                   # 擴展功能 (存在但未顯示檔案)
├── 🧪 NuTrackDemo03UITests/             # UI 測試層
│   ├── NuTrackDemo03UITests.swift       # 主要 UI 測試
│   └── NuTrackDemo03UITestsLaunchTests.swift # 啟動效能測試
└── 📋 Info.plist                        # 應用程式資訊設定 (可能存在)
```

## 🏗️ 各層結構詳細說明

### 1. 專案設定層 (Project Configuration)
**路徑：** `NuTrackDemo03.xcodeproj/`

```xml
<!-- xcschememanagement.plist -->
<key>NuTrackDemo03.xcscheme_^#shared#^_</key>
<dict>
    <key>orderHint</key>
    <integer>0</integer>
</dict>
...
**用途：**
- **編譯配置：** 管理不同的建置方案 (Debug/Release)
- **目標設定：** 定義編譯目標和相依性
- **使用者設定：** 儲存個人化的 Xcode 偏好設定
- **版本控制：** 專案設定的版本追蹤
...
### 5. 測試層 (Testing)
**路徑：** `NuTrackDemo03UITests/`

#### 5.1 UI 功能測試
**檔案：** `NuTrackDemo03UITests.swift`
...
**用途：**
- **功能驗證：** 確保 UI 組件正常運作
- **使用者流程測試：** 驗證完整的使用者操作流程
- **回歸測試：** 防止新功能破壞現有功能
- **品質保證：** 維持應用程式的品質標準

#### 5.2 效能測試
**檔案：** `NuTrackDemo03UITestsLaunchTests.swift`
